...
public class SettingsActivity extends PreferenceActivity {
	...
	protected void onCreate(Bundle savedInstanceState){
		...
		if( MainActivity.sMainActivity != null && MainActivity.sMainActivity.mConfig != null){
			...
			List<CaptureVideoResolution> resList = config.getVideoSupportedRes();
			for(CaptureVideoResolution vr : resList){
				String svr = ""+((config.getVideoOrientation() == 0)?config.getVideoWidth(vr):config.getVideoHeight(vr));
				if(config.getVideoResolution() == CaptureVideoResolution.VR_720x576){
					svr = "721";
				}
				int i;
